...
export function LatestSessionPhoto() {
  ...
  if (loading || !active) {
    return (
      <div className="brand-surface relative flex min-h-44 items-center justify-center border-t border-[hsl(var(--band-foreground))]/25 px-4 py-12 sm:min-h-56">
        {loading ? (
          <div className="h-28 w-full max-w-5xl animate-pulse-arena bg-[hsl(var(--band-foreground))]/10" />
        ) : (
  ...
  return (
    <div className="brand-surface relative border-t border-[hsl(var(--band-foreground))]/25">
      <div className="relative h-[clamp(15rem,38vw,34rem)] overflow-hidden">
        <img
          key={active.id}
        ...
        <div className="absolute inset-x-0 bottom-0">
          <div className="container flex items-end justify-between gap-5 pb-5 sm:pb-7">
      ...
      {photoSessions.length > 1 && (
        <div className="container flex gap-2 overflow-x-auto py-3 no-scrollbar">
          {photoSessions.map((session, index) => (
            ...
            >
              <img src={session.group_photo_url!} alt="" className="h-full w-full object-cover" loading="lazy" />
            </button>
          ))}
        </div>
      )}
